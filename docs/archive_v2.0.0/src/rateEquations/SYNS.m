...
for kk = 1:length(activators)
    ...
    new_str1 = strcat('ksyn_',species{2},'_',activators{kk},'*',activators{kk});
...
if ~isempty(inhibitors)
    ...
    for kk = 1:length(inhibitors)
        str_px = strcat('(','1 + Ki_syn_',species{2},'_',species{3},'_',inhibitors{kk},'*',inhibitors{kk},')*');
...
for kk = 1:length(inhibitors)
    Process(ii).param{rr+1} = strcat('Ki_syn_',species{2},'_',species{3},'_',inhibitors{kk});
    rr = rr+1;
end
