...
Process(ii).species = {species{2}};




str_p1 = strcat('vf = ','kdeg_',species{2},'*',species{2},'/(');
...
if ~isempty(inhibitors)
    ...
    for kk = 1:length(inhibitors)
        str_px = strcat('(','1 + Ki_deg_',species{2},'_',inhibitors{kk},'*',inhibitors{kk},')*');
...
Process(ii).param{1} = strcat('kdeg_',species{2});


rr = length(Process(ii).param);
for kk = 1:length(inhibitors)
    Process(ii).param{rr+1} = strcat('Ki_deg_',species{2},'_',inhibitors{kk});
    rr = rr+1;
end
