%% Michaelis-Menten - Full (Reversible) } A <=> B
...
for kk = 1:length(activators)
    ...
    %     new_str1 = strcat('*',activators{kk});
    %     str_star = strcat(str_star,new_str1);
...
str_p1 = strcat('vf = (',str_react,' * ',species{2},...
    ' / ','(','Km_',species{2},'_',species{3},'_',str_param,' + ',species{2},')/(');
...
str_p3 = strcat(' - ','Vm_',species{3},'_',species{2},' * ',species{3},...
    ' / ','(','Km_',species{3},'_',species{2},' + ',species{3},')');
...
Process(ii).param{rr+1} = strcat('Km_',species{2},'_',species{3},'_',str_param);
Process(ii).param{rr+2} = strcat('Vm_',species{3},'_',species{2});
Process(ii).param{rr+3} = strcat('Km_',species{3},'_',species{2});
