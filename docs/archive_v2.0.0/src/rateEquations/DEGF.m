...
str_den = strcat('(Km_',species{2},'_',species{3},' + ',species{2},')');
str_p1 = strcat('vf = (',str_react,')/',str_den,'/(');
...
rr = length(Process(ii).param);
for  kk = 1:length(inhibitors)
    Process(ii).param{rr+1} = strcat('Ki_deg_',species{2},'_',species{3},'_',inhibitors{kk});
    rr = rr+1;
end
