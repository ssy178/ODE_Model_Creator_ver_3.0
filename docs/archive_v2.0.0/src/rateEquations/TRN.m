...
str_p1 = strcat('vf = ','ktrn_',species{2},'_',species{3},'*',species{2},'/(');

% inhibitors --
if ~isempty(inhibitors)

    for kk = 1:length(inhibitors)
        str_px = strcat('(','1 + Ki_trn_',species{2},'_',species{3},'_',inhibitors{kk},'*',inhibitors{kk},')*');
...
str_p3 = strcat('-','ktrn_',species{3},'_',species{2},'*',species{3});

Process(ii).rate_eq = [str_p1 str_p3];


Process(ii).param{1} = strcat('ktrn_',species{2},'_',species{3});
Process(ii).param{2} = strcat('ktrn_',species{3},'_',species{2});
rr = length(Process(ii).param);

for kk = 1:length(inhibitors)
    Process(ii).param{rr+1} = strcat('Ki_trn_',species{2},'_',species{3},'_',inhibitors{kk});
    rr = rr+1;
end
