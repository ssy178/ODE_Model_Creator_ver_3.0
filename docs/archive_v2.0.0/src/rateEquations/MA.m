%% MASS ACTION TYPE (Reversible):  A + B <=> C
...
str_p2         = strcat(' - ',' kd_',species{4},'_',species{2},'_',species{3},'*',species{4});
...
rr = length(Process(ii).param);
Process(ii).param{rr+1}    = strcat('kd_',species{4},'_',species{2},'_',species{3});
